...
<template>
  <div class="add">
    ...
    <el-tab-pane name="first" :model="postChange" :rules="rules">
...
<!--          <li v-if="optionValue == '选择题'">
          ...
          <li v-if="optionValue == '选择题'">
            <span>正确选项:</span>
            <el-select v-model="postChange.correctAnswer" placeholder="选择正确答案" class="w150">
              <el-option
...
<!--          <li>
          ...
          <li>
            <span>题目数量：</span> <el-input type="text" v-model="changeNumber"></el-input>
          </li>
<!--          <li>
...
<script>
import {addTest, updateTest, getTest} from "@/api/monitor/test";
...
export default {
  ...
  methods: {
    loadData() {
      getTest(this.editId).then(res => {
        this.postChange = res.data;
    ...
    changeSubmit: function() {
      if (this.postChange.questionId != undefined) {
        updateTest(this.postChange).then(res => {
          this.$modal.msgSuccess("修改成功");
          this.$router.push('/uniapp/test')
        });
      } else {
        addTest(this.postChange).then(res => {
